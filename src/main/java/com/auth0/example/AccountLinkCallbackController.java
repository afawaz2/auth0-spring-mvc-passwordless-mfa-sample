...
@Controller
public class AccountLinkCallbackController extends Auth0CallbackHandler
    ...
    @RequestMapping(value="/acallback", method = RequestMethod.GET)
    ...
    protected Auth0User handleAccountLink(final Auth0User user, final HttpServletRequest req, final Tokens tokens) {
        ...
        if (auth0.isDropBoxAuth0User(user) && !isLinkedAccount(user)) {
            final Auth0User existingUser = SessionUtils.getAuth0User(req);
            final Tokens existingTokens = SessionUtils.getTokens(req);
            if (existingUser != null && auth0.isEmailAuth0User(existingUser))  {
                return auth0.linkAccount(existingUser, tokens, existingTokens);
            }
        }
